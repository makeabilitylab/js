/**
 * Makeability Lab Logo — Z-Zoom animation demo.
 *
 * A grid of triangles flies in from the front of the screen: each piece starts
 * overscaled and shrinks down to settle onto the screen plane, fading in as it
 * lands. After the grid starts filling in, the Makeability Lab logo's own pieces
 * zoom in the same way — each logo triangle and each individual segment of the M
 * and L outlines — until the finished logo is composed on top of the aligned
 * grid. Built on the makelab logo library and the raw Canvas 2D API (no p5.js).
 *
...
function buildScene() {
  ...
  const gridVisible = zoomFade ? zoomFade.grid.visible : true;
  ...
  zoomFade = new MakeabilityLabLogoZoomFade(makeLabLogo, w, h);
  zoomFade.grid.visible = gridVisible;

  // Expose for console tinkering.
  window.makeLabLogo = makeLabLogo;
  window.zoomFade = zoomFade;
}
